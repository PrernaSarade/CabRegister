...
class srchTable extends JFrame implements ActionListener
{
	...
	//JLabel StatusLabel = new JLabel("CREATED BY MANEESH AND VINAYAK TYBCS :CSIT COLLAGE",JLabel.CENTER);
	...
	public srchTable(String Did,String Dtype)
    {
		...
		try
		{
			...
			if(Dtype=="Driver ID")
			{	str= "SELECT * FROM LogTable WHERE D_No=\'"+Did+"\'";
			    n=1;
			}
			else if(Dtype == "Date")
			{	str= "SELECT * FROM LogTable WHERE Date=\'"+Did+"\'";
			    n=4;
			}
			else if(Dtype == "Driver Name")
			{
					str= "SELECT * FROM LogTable WHERE D_Name=\'"+Did+"\'";
					n=2;
			}
			else if(Dtype == "Cab No")
			{
				str= "SELECT * FROM LogTable WHERE C_No=\'"+Did+"\'";
				n=3;
			}
		...
		System.out.println("while");
			...
			if(flag==true)
			{
			    new SplashScreen().SetMessage(1);
				Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
				c = DriverManager.getConnection("jdbc:odbc:Database");
				s = c.createStatement();
				rs = s.executeQuery(str);

				String[] colHeads={"Driver No","Driver Name","Cab Number","Time In","Time Out","Date","Meter In","Meter Out"};
				int i=0,j=0;
				int count =0;
				while(rs.next())
				{
	...
	{
		JBSearch.setToolTipText("Go Back To Search Field And Search For Another Record");
		JBPrint.setToolTipText("Print The Table");
		JBExit.setToolTipText("Exit To Main Admin Page");
	}
}
...
public class Search extends JDialog
{
	...
	JLabel JLBanner = new JLabel("Selct what to locate and select where to locate.");
